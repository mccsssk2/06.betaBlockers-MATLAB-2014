%===============================================================================
% CellML file:   Z:\home\DATA\tmp\courtemanche_ramirez_nattel_1998-43c29fd00a0d\courtemanche_ramirez_nattel_1998.cellml
% CellML model:  courtemanche_1998
% Date and time: 2/1/2012 at 9:27:33 at AM
%-------------------------------------------------------------------------------
...
function dY = dy_crn(time, Y,runType)
global Istim stim;
global gtomult;
global gk1mult;
...
% AF=0; % done roughly according to Grandi 2011.

%**********************************************
% Stimulus
%**********************************************
if (nargin == 3)
    if strcmp(runType,'cond')
        ...
        st = stim( find(stim(:,1) <= time & stim(:,1) >= 0) , 2 ); % Find all (time, current)-data which t is smaller or equal to t
        if isempty(st)
            i_st = 0;
        else
            i_st = st(end); % use the last value (highest t)
        end
    end
else
    i_st = Istim;
...
K_rel = 30.0 + 60*AF;   % per_millisecond (in Ca_release_current_from_JSR)
...
g_Ca_L = (1-0.5*AF)*0.12375;   % nanoS_per_picoF (in L_type_Ca_channel)
I_NaCa_max = (1+0.4*AF)*1600.0;   % picoA_per_picoF (in Na_Ca_exchanger_current)
...
g_Na = 7.8*(1-0.1*AF);   % nanoS_per_picoF (in fast_sodium_current)
...
g_Ks = 0.12941176*(1+AF);   % nanoS_per_picoF (in slow_delayed_rectifier_K_current)
...
g_K1 = gk1mult*0.09*(1+AF);   % nanoS_per_picoF (in time_independent_potassium_current)
tau_tr = 180.0;   % millisecond (in transfer_current_from_NSR_to_JSR)
K_Q10 = 3.0;   % dimensionless (in transient_outward_K_current)
g_to = gtomult*0.1652*(1.0-0.7*AF);   % nanoS_per_picoF (in transient_outward_K_current)
...
i_up_leak = (1.0+0.25*AF)*I_up_max*Y(12)/Ca_up_max;
V_rel = 0.0048*V_cell;
...
% Late I_Na
GNaL=0.0025*AF;
aml = 0.32*(Y(15)+47.13)/(1-exp(-0.1*(Y(15)+47.13)));
bml = 0.08*exp(-Y(15)/11);
hlinf = 1/(1+exp((Y(15)+91)/6.1));
tauhl=600;
dY(22,1) = aml*(1-Y(22))-bml*Y(22);
dY(23,1) = (hlinf-Y(23))/tauhl;
% 
I_NaL_junc = Fjunc*GNaL*Y(22)^3*Y(23)*(Y(15)-E_Na);
I_NaL_sl = Fsl*GNaL*Y(22)^3*Y(23)*(Y(15)-E_Na);
...
dY(14, 1) = (-3.0*i_NaK-(3.0*i_NaCa+i_B_Na+i_Na+I_NaL))/(V_i*F);
i_K1 = Cm*g_K1*(Y(15)-E_K)/(1.0+exp(0.07*(Y(15)+80.0)));
i_to = Cm*g_to*Y(18)^3.0*Y(19)*(Y(15)-E_K);
g_Kur = (0.005+0.05/(1.0+exp((Y(15)-15.0)/-13.0)))*(1.0-0.5*AF);
...
dY(15, 1) = -(i_Na+i_K1+i_to+i_Kur+i_Kr+i_Ks+i_B_Na+i_B_Ca+i_NaK+i_CaP+i_NaCa+i_Ca_L+i_st + I_NaL)/Cm;
