...
for AF_it=1:1:1
for BB=1:1:2
for cellType=2:2:2 % 1 = CRN, 2 = Grandi, 3 = KT
...
clear apchar;
%
cycle_file=cycle(end-8:end-2);
apd30_file=apd30(end-8:end-2);
apd50_file=apd50(end-8:end-2);
apd90_file=apd90(end-8:end-2);
cycle_file=cycle(end-8:end-2);
...
clear apchar;
%
os_file=overshoot(end-8:end-2);
rest_file=resting(end-8:end-2);
amp_file=os_file - rest_file;
v30_file=apd30(end-8:end-2)';
v50_file=apd50(end-8:end-2)';
v90_file=apd90(end-8:end-2)';
maxCai_file=maxCai(end-8:end-2);
minCai_file=minCai(end-8:end-2);
size(cycle_file)
size(os_file)
size(rest_file)
size(amp_file)
